...
const OLevelPage: React.FC = () => {
  ...
  const filteredAndSortedSubjects = React.useMemo(() => {
    ...
    if (filterBy !== 'all') {
      if (filterBy === 'science') {
        filtered = subjects.filter(s => ['biology', 'chemistry', 'physics', 'comb-sci'].includes(s.id));
      } else if (filterBy === 'languages') {
        filtered = subjects.filter(s => ['urdu-1', 'urdu-2', 'english', 'english-lit'].includes(s.id));
      } else if (filterBy === 'business') {
        filtered = subjects.filter(s => ['business', 'economics', 'commerce', 'accounting'].includes(s.id));
      } else if (filterBy === 'social') {
        filtered = subjects.filter(s => ['pak-studies', 'islamiyat', 'sociology', 'global-persp'].includes(s.id));
      }
    }
